 ...
 * Removes quarantine attributes from all .node files in node_modules
 */
...
function removeQuarantine(dir) {
  try {
    ...
    // Find all .node files and remove quarantine
    const findCommand = `find "${dir}" -name "*.node" -type f 2>/dev/null`;
    try {
      ...
      // No .node files found or find failed
    }
  } catch (error) {
    // Ignore errors
  }
